...
function authHeader() {
  return state.auth
    ? {
        Authorization: "Basic " + btoa(`${state.auth.user}:${state.auth.pass}`),
      }
    : {};
}
...
function resolveFrameImageUrl(frameUrl) {
  if (!frameUrl) return "";
  const value = String(frameUrl);
  if (isAbsoluteUrl(value)) return value;
  if (value.startsWith("/api/")) return value;
  if (value.startsWith("/camera-system/")) return `${state.base}${value}`;
  if (value.startsWith("camera-system/")) return `${state.base}/${value}`;
  return value.startsWith("/") ? value : `${state.base}/camera-system/${value}`;
}
...
function saveAuth() {
  localStorage.setItem("cambotAuth", JSON.stringify(state.auth));
}
function loadAuth() {
  try {
    state.auth = JSON.parse(localStorage.getItem("cambotAuth") || "null");
  } catch {
    state.auth = null;
  }
}
...
async function refreshAll() {
  ...
  await loadBindings();
  await loadPromptCameraGroups();
  renderGroupSelects();
  renderPromptFilters();
  renderPrompts();
  await loadResultOperations();
  await loadResults();
}

function selectInitialPromptAndGroup() {
  if (!state.selectedPromptId && state.prompts.length)
    state.selectedPromptId = state.prompts[0].id;
  ensureSelectedResultGroup();
}
  ...
  for (const group of state.groups) {
    try {
      const data = await api(
        `/camera-groups/${encodeURIComponent(group.id)}/prompt-bindings`,
      );
...
async function loadPromptCameraGroups(promptId = state.selectedPromptId) {
  if (!promptId) return [];
  try {
    const data = await api(
      `/saved-prompts/${encodeURIComponent(promptId)}/camera-groups`,
    );
    const groups = (data.groups || []).map(normalizeGroup);
    state.promptGroups.set(promptId, groups);
    return groups;
  } catch {
    state.promptGroups.delete(promptId);
    return [];
  }
}
...
function getModalPrompt() {
  const el = $("promptId");
  return el?.value ? getPromptById(el.value) : null;
}
function getSelectedGroup() {
  return state.groups.find((g) => g.id === state.selectedGroupId) || null;
}
function getPromptBindings(promptId) {
  return state.bindings.filter(
    (b) => b.promptId === promptId && b.enabled !== false,
  );
}
function getBinding(promptId, groupId) {
  return (
    state.bindings.find(
      (b) =>
        b.promptId === promptId &&
        b.cameraGroupId === groupId &&
        b.enabled !== false,
    ) || null
  );
}
...
function isPromptScanning(promptId) {
  return getRunningOperations(promptId).length > 0;
}

function promptMatchesFilters(prompt) {
  const bindings = getPromptBindings(prompt.id);
  if (
    state.promptGroupFilter &&
    !bindings.some((b) => b.cameraGroupId === state.promptGroupFilter)
  )
    return false;
  ...
  const matchingCameraIds = new Set(
    state.cameras
      .filter((c) =>
        [c.id, c.name, c.location].some((v) =>
          String(v || "")
            .toLowerCase()
            .includes(q),
        ),
  ...
  const matchingGroupIds = new Set(
    state.groups
      .filter((g) =>
        (g.cameraIds || []).some((id) => matchingCameraIds.has(id)),
      )
      .map((g) => g.id),
  );
...
function renderPromptFilters() {
  $("promptGroupFilter").innerHTML =
    '<option value="">All scheduled groups</option>' +
    state.groups
      .map(
        (g) =>
          `<option value="${esc(g.id)}">${esc(g.name)} · ${g.cameraIds.length} cams</option>`,
      )
      .join("");
...
function updatePromptFilterSummary() {
  ...
  if (state.promptCameraFilter.trim())
    parts.push(
      `affecting cameras matching “${state.promptCameraFilter.trim()}”`,
    );
...
function renderPrompts() {
  ...
  if (!state.prompts.length) {
    list.innerHTML =
      '<div class="empty">No prompts yet. Create a new prompt to begin.</div>';
    return;
  }
  if (!prompts.length) {
    list.innerHTML =
      '<div class="empty">No prompts match the current filters.</div>';
    return;
  }
  ...
  list.innerHTML = prompts
    .map((prompt) => {
      ...
      const groupNames = bindings
        .map(
          (b) =>
            state.groups.find((g) => g.id === b.cameraGroupId)?.name ||
            b.cameraGroupId,
        )
        .join(", ");
      const runningGroups = getRunningOperations(prompt.id)
        .map(
          (o) =>
            state.groups.find((g) => g.id === o.cameraGroupId)?.name ||
            o.cameraGroupId,
        )
        .filter(Boolean)
        .join(", ");
        ...
        <div class="prompt-main">
          <div class="prompt-title-line">
            ...
            <span class="prompt-badges">
              ...
              ${running ? `<span class="pill warn" title="Running: ${esc(runningGroups)}">Scanning</span>` : ""}
...
function resultGroupsForPrompt(promptId = state.selectedPromptId) {
  if (!promptId) return [];
  const contractGroups = state.promptGroups.get(promptId);
  if (contractGroups?.length) return contractGroups;

  // Fallback keeps the UI usable if the deployed server is older than the contract.
  const ids = new Set();
  state.bindings
    .filter((b) => b.promptId === promptId && b.enabled !== false)
    .forEach((b) => ids.add(b.cameraGroupId));
  state.operations
    .filter((o) => o.promptId === promptId && o.cameraGroupId)
    .forEach((o) => ids.add(o.cameraGroupId));
  return state.groups.filter((g) => ids.has(g.id));
}

function ensureSelectedResultGroup() {
  const groups = resultGroupsForPrompt();
  if (!groups.length) {
  ...
  if (
    ...
    !groups.some((g) => g.id === state.selectedGroupId)
  ) {
    state.selectedGroupId = groups[0].id;
  }
}

async function loadResultOperations() {
  ...
  if (select)
    select.innerHTML = '<option value="latest">Most recent pass</option>';
  if (!prompt || !group) return;
  try {
    const data = await api(
      `/operations?promptId=${encodeURIComponent(prompt.id)}&cameraGroupId=${encodeURIComponent(group.id)}&limit=25&offset=0`,
    );
    state.resultOperations = (data.operations || []).map(normalizeOperation);
    if (select) {
      const options = [
        '<option value="latest">Most recent pass</option>',
      ].concat(
        state.resultOperations.map(
          (o) =>
            `<option value="${esc(o.id)}">${esc(nowish(o.createdAt))} · ${esc(o.status)} · ${esc(o.trigger || "manual")}</option>`,
        ),
      );
    ...
    // Operation history is optional for the viewer; latest results can still load.
  }
}
...
function renderGroupSelects() {
  ...
  const options = groups
    ...
    .join("");
  $("resultGroupSelect").innerHTML =
    options || '<option value="">No used camera groups</option>';
  if (
    state.selectedGroupId &&
    [...$("resultGroupSelect").options].some(
      (o) => o.value === state.selectedGroupId,
    )
  )
    $("resultGroupSelect").value = state.selectedGroupId;
}
...
async function openPromptModal(prompt = null) {
  ...
  const isNew = !prompt;
  $("promptModalTitle").textContent = isNew
    ? "Create New Prompt"
    : "Edit Prompt";
  $("modalSubtitle").textContent = isNew
    ? "Create the prompt, then save it before running or scheduling."
    : "Edit the prompt and manage each camera group schedule separately.";
...
function isActionLoading(kind, groupId = "") {
  return state.loadingActions.has(actionKey(kind, groupId));
}
function anyScheduleActionLoading() {
  return Array.from(state.loadingActions).some(
    (k) =>
      k.startsWith("schedule:") ||
      k.startsWith("removeSchedule:") ||
      k.startsWith("runScan:"),
  );
}
...
async function withLoadingAction(kind, groupId, message, fn) {
  ...
  try {
    ...
    setTimeout(() => {
      if (!anyScheduleActionLoading()) setModalStatus("");
    }, 1100);
    return result;
  } catch (err) {
...
async function savePrompt() {
  ...
  const saved = id
    ? await api(`/saved-prompts/${encodeURIComponent(id)}`, {
        method: "PUT",
        body: JSON.stringify(payload),
      })
    : await api("/saved-prompts", {
        method: "POST",
        body: JSON.stringify(payload),
      });
  await loadPrompts();
  state.selectedPromptId =
    saved?.id ||
    id ||
    state.prompts.find((p) => p.name === payload.name)?.id ||
    state.selectedPromptId;
...
async function deletePrompt() {
  ...
  await api(`/saved-prompts/${encodeURIComponent(prompt.id)}`, {
    method: "DELETE",
  });
  closePromptModal();
...
async function runSingleScanForGroup(groupId) {
  ...
  await withLoadingAction(
    "runScan",
    group.id,
    `Queueing single scan for ${group.name}...`,
    async () => {
      const operation = await api("/operations", {
        method: "POST",
        body: JSON.stringify({
          promptId: prompt.id,
          cameraGroupId: group.id,
          trigger: "manual",
        }),
      });
      ...
      renderPrompts();
      setModalStatus(
        `Queued single scan for ${group.name}: ${operation?.id || "operation created"}`,
        "success",
      );
    },
  );
...
async function enableContinuousForGroup(groupId) {
  ...
  await withLoadingAction(
    "schedule",
    group.id,
    `Scheduling ${prompt.name || prompt.id} on ${group.name}...`,
    async () => {
      await api(
        `/camera-groups/${encodeURIComponent(group.id)}/prompt-bindings`,
        {
      ...
      await loadBindings();
      await loadPromptCameraGroups(prompt.id);
      renderPrompts();
      renderGroupSelects();
    },
  );
...
async function disableContinuousForGroup(groupId) {
  ...
  await withLoadingAction(
    "removeSchedule",
    group.id,
    `Removing background schedule for ${group.name}...`,
    async () => {
      await api(
        `/camera-groups/${encodeURIComponent(group.id)}/prompt-bindings/${encodeURIComponent(binding.id)}`,
        { method: "DELETE" },
      );
      await loadBindings();
      await loadPromptCameraGroups(prompt.id);
...
function renderModalScheduleList() {
  ...
  if (!prompt) {
    list.innerHTML =
      '<div class="empty">Save this prompt before running or scheduling it.</div>';
    return;
  }
  ...
  list.innerHTML = state.groups
    .map((group) => {
      ...
      const running = state.operations.some(
        (o) =>
          o.promptId === prompt.id &&
          o.cameraGroupId === group.id &&
          [
            "queued",
            "running",
            "first_pass_running",
      ...
      <article class="schedule-row ${busy ? "busy" : ""}" data-schedule-group-id="${esc(group.id)}" aria-busy="${busy ? "true" : "false"}">
        ...
        <div class="schedule-status">
          ...
          ${running ? '<span class="pill warn">Scanning</span>' : ""}
  ...
  document.querySelectorAll(".run-group-btn").forEach((btn) => {
    btn.onclick = () =>
      guard(() => runSingleScanForGroup(btn.dataset.runGroupId));
  });
  document.querySelectorAll(".schedule-group-btn").forEach((btn) => {
    btn.onclick = () =>
      guard(() => enableContinuousForGroup(btn.dataset.scheduleGroupId));
  });
  document.querySelectorAll(".remove-schedule-btn").forEach((btn) => {
    btn.onclick = () =>
      guard(() => disableContinuousForGroup(btn.dataset.removeScheduleGroupId));
  });
}

async function imageObjectUrl(frameUrl) {
  if (!frameUrl) return null;
  const resolvedUrl = resolveFrameImageUrl(frameUrl);
  if (state.imageUrls.has(resolvedUrl)) return state.imageUrls.get(resolvedUrl);
  try {
    const response = await fetch(resolvedUrl, { headers: authHeader() });
    if (!response.ok) return null;
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    state.imageUrls.set(resolvedUrl, url);
    return url;
  } catch {
    return null;
  }
...
function normalizeResult(r) {
  return {
    ...
    promptScore:
      r.promptScore ??
      r.prompt_score ??
      r.firstPassPromptScore ??
      r.first_pass_prompt_score,
...
async function loadResults() {
  const prompt = getSelectedPrompt();
  const group = getSelectedGroup();
  const passLabel = state.showingFirstPass
    ? "first pass/intermediate"
    : "second pass/global";
  const opLabel =
    state.selectedOperationId && state.selectedOperationId !== "latest"
      ? "selected previous pass"
      : "most recent pass";
  $("resultContext").textContent =
    prompt && group
      ? `${prompt.name} · ${group.name} · ${passLabel} results · ${opLabel}`
      : "Select a prompt and a camera group used by that prompt.";
  if (!prompt || !group) {
    $("resultList").innerHTML =
      '<div class="empty">Select a prompt with a scheduled or previously run camera group to see results.</div>';
    return;
  }
  $("resultList").innerHTML =
    '<div class="empty"><span class="spinner"></span>Loading results...</div>';
  const latestPass = state.showingFirstPass ? "first-pass" : "second-pass";
  const operationPass = state.showingFirstPass
    ? "first-pass-results"
    : "second-pass-results";
  try {
    const path =
      state.selectedOperationId && state.selectedOperationId !== "latest"
        ? `/operations/${encodeURIComponent(state.selectedOperationId)}/${operationPass}`
        : `/prompt-results/latest/${latestPass}?promptId=${encodeURIComponent(prompt.id)}&cameraGroupId=${encodeURIComponent(group.id)}`;
    const data = await api(path);
    const results = (data.results || []).map(normalizeResult);
    await renderResults(results);
  } catch (err) {
    $("resultList").innerHTML =
      `<div class="empty">Could not load results yet.<br><small>${esc(err.message || err)}</small></div>`;
  }
}
...
async function renderResults(results) {
  if (!results.length) {
    $("resultList").innerHTML =
      '<div class="empty">No results yet. Run a single scan or wait for the background scanner.</div>';
    return;
  }
  ...
  for (const card of document.querySelectorAll(".result-card")) {
    ...
    slot.innerHTML = url
      ? `<img src="${esc(url)}" alt="camera snapshot">`
      : "<span>No snapshot image</span>";
  }
}
...
async function loadQueue() {
  try {
    ...
    $("queueList").innerHTML = items.length
      ? items
          .map((item) => {
            ...
            const score =
              item.operatorPriorityScore ?? item.operator_priority_score;
    ...
    $("queueList").innerHTML =
      `<div class="empty">Could not load queue.<br><small>${esc(err.message || err)}</small></div>`;
  }
}
...
async function loadSettings() {
  try {
    ...
    $("continuousEnabled").checked = Boolean(
      s.continuousScanEnabled ?? s.continuous_scan_enabled,
    );
    $("continuousInterval").value =
      s.continuousScanIntervalSeconds ??
      s.continuous_scan_interval_seconds ??
      900;
    $("geminiDelay").value =
      (s.geminiCallDelayMs ?? s.gemini_call_delay_ms ?? 2000) / 1000;
    $("maxCostDay").value =
      s.maxEstimatedCostPerDay ?? s.max_estimated_cost_per_day ?? "";
    $("nextScanAt").textContent = nowish(
      s.nextContinuousScanAt ?? s.next_continuous_scan_at,
    );
  } catch {
...
async function saveSettings() {
  await api("/settings/gemini", {
    ...
    body: JSON.stringify({
      ...
      continuousScanIntervalSeconds: Number(
        $("continuousInterval").value || 900,
      ),
      geminiCallDelayMs: Number($("geminiDelay").value || 2) * 1000,
...
async function loadUsage() {
  try {
    ...
    $("prevDayCost").textContent = fmtMoney(
      u.previousDayCost ?? u.previous_day_cost,
    );
    $("monthCost").textContent = fmtMoney(
      u.monthToDateCost ?? u.month_to_date_cost,
    );
    $("projectedCost").textContent = fmtMoney(
      u.projectedCostPerDay ?? u.projected_cost_per_day,
    );
...
function wireEvents() {
  ...
  $("password").onkeydown = (e) => {
    if (e.key === "Enter") guard(login);
  };
  $("logoutBtn").onclick = logout;
  $("refreshPromptsBtn").onclick = () =>
    guard(async () => {
      await Promise.all([
        loadPrompts(),
        loadCameras(),
  ...
  $("promptGroupFilter").onchange = () => {
    state.promptGroupFilter = $("promptGroupFilter").value;
    renderPrompts();
  };
  $("promptCameraFilter").oninput = () => {
    state.promptCameraFilter = $("promptCameraFilter").value;
    renderPrompts();
  };
  $("clearPromptFiltersBtn").onclick = () => {
    state.promptGroupFilter = "";
    state.promptCameraFilter = "";
    renderPromptFilters();
    renderPrompts();
  };
  $("resultGroupSelect").onchange = async () => {
    state.selectedGroupId = $("resultGroupSelect").value;
    state.selectedOperationId = "latest";
    await loadResultOperations();
    await loadResults();
  };
  ...
  $("resultPassSelect").onchange = async () => {
    state.selectedOperationId = $("resultPassSelect").value || "latest";
    await loadResults();
  };
  $("showFirstPassBtn").onclick = () =>
    guard(async () => {
      state.showingFirstPass = !state.showingFirstPass;
      $("showFirstPassBtn").textContent = state.showingFirstPass
        ? "Show Second Pass"
        : "Show First Pass";
      await loadResults();
    });
